...
import { createUser, getCertification, getMajors } from '../../component/service/Data';
...
export const NewTeacher = () => {
  ...
  * @Param value
  */
  const submitConfirmation = (values: any) => {
  ...
  /**
  * show the add confirm to open the NewTeacher Page
  * @Param value
  */
  const showAddConfirm= (value: any) => {
    confirm({
      title: 'You want to add this teacher?',
  ...
  */
  ...
  useEffect(() => {
    if (status === RequestStatus.COMPLETED) {
      ...
      confirm({
        title:data,
        content: "Do you want to add more teacher?" ,
        okText: 'Yes',
        okType: 'danger',
        cancelText: 'No',
        onOk: () => {
    ...
    <Layout id='components-layout-demo-custom-trigger' style={{ minHeight: '100vh' }}>
      ...
      <Space className="contain-f" direction="horizontal">
        <div className="edit-f-contain">
          <div className="title form-title" style={{ textAlign: 'center' }}> NEW TEACHER </div>
        <Form
        ...
        >
           <Form.Item
            ...
            <Form.Item name="certificationID"
              label="Certification"
              colon={false}
              rules={[validRule.certification]}>
              <Select style={{ textAlign: "left" }} placeholder="Select the certification" >
                {
