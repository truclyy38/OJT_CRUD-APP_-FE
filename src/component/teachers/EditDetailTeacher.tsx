...
import { editUserById, getCertification, getMajors, getUserByID } from "../service/Data";

import moment from "moment";
import { ICertification } from "../../@types/ICertification";

const { Option } = Select;
let teacherInput: IUser;
 ...
 */
export const EditDetailTeacher = (props: any) => {
  ...
  function showConfirmation(value: any) {
    confirm({
      title: 'Are you sure update this teacher?',
  ...
  const submitChange = (fieldValues: any) => {
    ...
    fieldValues.id = params.teacherId;

    // no define teacherCode
    fieldValues.studentCode = 0;
    // fieldValues.certificationId = 0;
    ...
    if (fieldValues.password === '') {
      fieldValues.password = teacherInput.password;
    }

    // check exist file image, if update new img, it will be posted to the firebase, return image url link and send request to api
    if (!file) {
...
/**
  * useEffect
  * @Param form, params.teacherId, status
  */
  useEffect(() => {
    if (status === RequestStatus.COMPLETED) {
      if (data == null) {
        ...
        confirm({
          title: data,
          content: "Do you want to back teacher page?",
          onOk: () => {
            history.push('/teachers');
          }
        });
    ...
    getUserByID(params.teacherId)
      .then((response: any) => {
        const teacherData = { ...response.data, birthday: moment(response.data.dob) }
        teacherData.password = "";
        form.setFieldsValue(teacherData);
        setImageUrl(response.data.image);
        teacherInput = response.data;
      })

    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form, params.teacherId, status])
    ...
    <Layout id='components-layout-demo-custom-trigger' style={{ minHeight: '100vh' }}>
      <Sider trigger={null} >
        <Menu theme="dark" mode="inline">
          ...
          <Menu.Item key="2" icon={<RightOutlined />}>
            <Link to={`/teachers`}>Students Information </Link>
          </Menu.Item>
      ...
      <Space className="contain-f" direction="horizontal">
        <div className="edit-f-contain">
          <div className="title form-title" style={{ textAlign: 'center' }}> EDIT TEACHER </div>
          ...
          >
            ...
            <Form.Item name="certificationID"
              label="Certification id"
              colon={false}
            >
              <Select placeholder="Select the certification">
                {
                  certifications.map((item: ICertification) => (
